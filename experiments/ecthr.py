...
def extend_longformer(config, model_args, max_pos=8192, attention_window=128, model_type=AutoModelForMaskedLM):
...
def main():
    ...
    if model_args.model_name_or_path == 'longformer-standard':
        ...
        model, tokenizer = extend_longformer(config=config, model_args=model_args, max_pos=8192, attention_window=128,
                                             model_type=AutoModelForSequenceClassification)

        assert model.config.max_position_embeddings == 8192
        assert model.longformer.embeddings.position_embeddings.weight.shape[0] == 8194
        assert tokenizer.model_max_length == 8192
    elif model_args.model_name_or_path == 'legal-longformer':
        raise NotImplementedError('Longformer warm-started from LEGAL-BERT is not implemented yet')
    ...
    def preprocess_function(examples):
        ...
        for case in examples['text']:
            cases.append(f' {tokenizer.sep_token} '.join([' '.join(paragraph.split()[:data_args.max_seg_length])
                                                          for paragraph in case[:data_args.max_segments]]))
